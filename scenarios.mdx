...
- **Resume** — click a draft to reopen the wizard at your last step.
- **Discard** — remove a draft you no longer need. Available for drafts and failed runs.
- **Cancel** — stop a run that's queued or generating.

A group can't be deleted while it has a draft or an in-progress (queued or generating) generation — discard or cancel those first. Completed and failed runs don't block deletion.
