...
Use the **Filter** button to narrow results by agent, creator, or status (Completed, Running, or Failed). Use the **Search** bar to filter by simulation name or agent name.
...
If no agents are connected, an inline banner links to the Agents page; if no scenarios exist, the sheet prompts you to create some first.
...
**Scenarios** — Select the scenarios to include. Scenarios are grouped by their scenario group in the selector. Toggle a whole group with its group checkbox, or use **Select All** / **Deselect All**. Use the search input and the **Created by me** filter to narrow a long list. Click a scenario name to preview its goal, profile, and knowledge base before selecting.
...
| Status | Done, Running, Pending, Failed, or Cancelled |
...
A simulation that is queued but has not started yet (**Pending**) can be cancelled. Open the kebab menu on that row and select **Cancel**; a toast confirms the cancellation.

Cancel is only offered for Pending simulations — once a run is Running it can no longer be cancelled.

---

## Rerunning a simulation

Open the kebab menu on a simulation row and select **Rerun**. Rerun is available once a simulation has started or finished — it is hidden while a run is still Pending and disabled while it is Running.

The rerun sheet pre-fills the simulation **name** (edit it to distinguish this run from the original) and notes that it reuses the same agent and the same set of scenarios. Click **Run Simulation** to start.
...
Open the kebab menu and select **Delete**, then confirm. Delete is disabled while a simulation is Running. The simulation and its conversations are removed.
...
<AccordionGroup>
  ...
  <Accordion title="How is completion rate calculated?">
    Completion rate is the percentage of conversations that reached a `Done` status. Conversations with a `Failed` status are counted as incomplete. Running conversations are excluded from the calculation until they finish.
  </Accordion>
