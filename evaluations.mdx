...
Expand a label row to see the conversations and turns where that label was assigned, then click a link to open the conversation modal at that turn.
...
Expand the **Reasoning** section on any assistant turn to read the LLM judge's explanation for that score — useful for identifying prompt improvements or metric calibration issues.
