...
Arkdock is a web platform for testing, simulating, and evaluating AI agents. It gives teams a central place to connect their agents, define realistic test scenarios, run multi-turn simulations, and score agent performance using LLM-judged evaluation metrics — all without writing test infrastructure from scratch.
...
flowchart LR
  ...
  subgraph score["3 · Score"]
    E["Evaluation\nLLM judge + metrics"]
    R["Results\nScores · Errors · Annotations"]
  end
  K -->|"grounds personas"| S
  A -->|"agent under test"| SIM
  S -->|"drives conversation"| SIM
  SIM -->|"transcripts"| E
  E --> R
```
...
Evaluations use your choice of LLM judge (GPT-4o, Claude, Gemini) to score conversations on built-in and custom metrics. Scores are computed at the conversation and metric level so you can compare runs, track regressions, and prioritize fixes.
...
Scenarios define the simulated user profiles that interact with your agent. Each scenario specifies a user goal, a persona description, and optionally a knowledge base of documents the simulated user can reference. Scenarios can be generated with AI or imported in bulk.

[Learn more about Scenarios](/scenarios)

### Simulations

A simulation runs a set of scenarios against a connected agent and records every conversation turn. You control how many conversations to generate per scenario and the maximum number of turns per conversation. Simulations can optionally trigger an evaluation automatically on completion.
...
<AccordionGroup>
  ...
  <Accordion title="Which LLM providers can be used as the evaluation judge?">
    You can choose from OpenAI (GPT-4o, GPT-4o Mini, GPT-4 Turbo, GPT-3.5 Turbo), Anthropic (Claude 3.5 Sonnet, Claude 3.5 Haiku, Claude 3 Opus, Claude 3 Haiku), and Google (Gemini 2.0 Flash, Gemini 1.5 Pro, Gemini 1.5 Flash).
  </Accordion>
  <Accordion title="Can I run an evaluation automatically after a simulation?">
    Yes. When creating a simulation you can enable "Also run an evaluation", select an LLM judge, and pick your metrics. The evaluation starts automatically once the simulation completes.
  </Accordion>
