...
Every completed evaluation has an **Annotations** tab — a spreadsheet with one row per conversation turn (or per conversation) and one column per metric. Each cell displays the LLM judge's auto score alongside any human annotation entered for that cell.
...
Comments are visible to all team members with access to the evaluation.
